...
public class PreventivoAlt {
	...
	private readonly FiguraGeometrica[] _prati_siepi;
	private readonly int _numElems = 0;

	private double _preventivoPrati;
	private double _preventivoSiepi;
	private double _preventivoTotale;

	public PreventivoAlt(int numElems) {
		this._prati_siepi = new FiguraGeometrica[numElems];
		this._numElems = numElems;
	}

	public void AggiungiElem(FiguraGeometrica fg) {
		for (int i = 0; i < this._numElems; i++) {
			if (this._prati_siepi[i] == null) {
				this._prati_siepi[i] = fg;
			}
		}
	}
	...
	public void CalcolaPreventivi() {
		...
		for (int i = 0; i < this._numElems; i++) {
			this._preventivoPrati += this._prati_siepi[i].Area() * this._pratoPrezzoMQ;
		}

		// Calcolo del preventivo per ogni siepe
		for (int i = 0; i < this._numElems; i++) {
			this._preventivoSiepi += this._prati_siepi[i].Perimetro() * this._siepePrezzoM;
		}
